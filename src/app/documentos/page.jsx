            ...
            {pedidos.length === 0 ? (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(400px, 1fr))', gap: '20px' }}>
                    {pedidos.map((pedido) => {
                        ...
                        return (
                            <div key={pedido.id} style={{ background: '#fff', padding: '20px', borderRadius: '8px', boxShadow: '0 4px 8px rgba(0,0,0,0.1)' }}>
                                ...
                                <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '20px' }}>
                                    <thead>
                                        <tr style={{ backgroundColor: '#f2f2f2' }}>
                                            <th style={{ padding: '8px', border: '1px solid #ddd', textAlign: 'left' }}>Cant.</th>
                                            <th style={{ padding: '8px', border: '1px solid #ddd', textAlign: 'left' }}>Detalle</th>
                                            <th style={{ padding: '8px', border: '1px solid #ddd', textAlign: 'right' }}>Unitario</th>
                                    ...
                                    <tbody>
                                        {(pedido.products || []).map((item, index) => (
                                            <tr key={index}>
                                                ...
                                                <td style={{ padding: '8px', border: '1px solid #ddd' }}>{`${item.product.nombre} ${item.product.peso}Kg`}</td>
                                                <td style={{ padding: '8px', border: '1px solid #ddd', textAlign: 'right' }}>{formatCurrency(item.product.selectedPrice.price)}</td>
                                                <td style={{ padding: '8px', border: '1px solid #ddd', textAlign: 'right' }}>{formatCurrency(item.product.selectedPrice.price * item.quantity)}</td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
